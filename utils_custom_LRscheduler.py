...
def run_training(model, optimizer, loss_function, device, num_epochs, 
    ...
    Args:
        ...
        val_dataloader (DataLoader): Torch DataLoader object to load the
            validation data
        stopper (EarlyStopper, optional): If passed, model will be trained
    ...
    for epoch in master_bar:
        ...
        # do a scheduler update
        if scheduler.finished():
            stopper.update_patience_spent(val_accs, epoch, model)
            if stopper.patience_spent > stopper.patience:
                stopper.load_model(model, epoch)
                return train_losses, val_losses, train_accs, val_accs, confusion_matrix

        # do an early stopper update
        else:
            scheduler.update_patience_spent(val_accs, epoch, model)
            if scheduler.patience_spent > scheduler.patience:
                scheduler.load_model(model, epoch)
                scheduler.step()
...
class  Scheduler(EarlyStopper):
    def __init__(self, patience, decays, torch_scheduler_object):
        super().__init__(patience=patience)
        self.decays = decays
        self.torch_scheduler_object = torch_scheduler_object
        self.decays_spent = 0

    def finished(self):
        return self.decays_spent == self.decays

    def step(self):
        self.torch_scheduler_object.step()
        self.patience_spent = 0
        self.decays_spent += 1
